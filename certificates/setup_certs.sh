...
# Create directories for Root CA
...
openssl req -x509 -new -nodes -key $ROOT_CA_DIR/rootCA.key -sha256 -days 3650 -out $ROOT_CA_DIR/rootCA.crt \
  -subj "/CN=Root CA"

# Create directories for Intermediate CA
...
EOF
openssl x509 -req -in $INTERMEDIATE_CA_DIR/csr/intermediateCA.csr -CA $ROOT_CA_DIR/rootCA.crt -CAkey $ROOT_CA_DIR/rootCA.key \
  -CAcreateserial -out $INTERMEDIATE_CA_DIR/certs/intermediateCA.crt -days 1825 -sha256 -extfile $INTERMEDIATE_CA_DIR/v3_intermediate.ext

# Generate CRL for Intermediate CA
echo "Generating CRL for Intermediate CA..."
openssl ca -gencrl -out $INTERMEDIATE_CA_DIR/crl/intermediateCA.crl -config openssl.cnf
...
openssl verify -CAfile <(cat $INTERMEDIATE_CA_DIR/certs/intermediateCA.crt $ROOT_CA_DIR/rootCA.crt) \
