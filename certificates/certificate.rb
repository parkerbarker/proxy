...
# class Certificate
#   class << self
#     SERIAL_FILE = File.join(File.dirname(__FILE__), "intermediate/serial")
...
#       FileUtils.mkdir_p(File.dirname(csr_path))
...
#       certificate.serial = next_serial
#       certificate.version = 2
#       certificate.not_before = Time.now
#       certificate.not_after = Time.now + 365 * 24 * 60 * 60  # 1 year
...
#       certificate.add_extension(ef.create_extension("authorityKeyIdentifier", "keyid,issuer", false))
#       certificate.add_extension(ef.create_extension("subjectAltName", "DNS:#{domain}", false))
...
#     # Generates and retrieves the next unique serial number
#     def next_serial
...
#         serial = 1000
#       end
#       File.write(SERIAL_FILE, (serial + 1).to_s)
#       serial
#     end
...
class Certificate
  class << self
    def generate(domain)
      ...
      certificate.add_extension(ef.create_extension("basicConstraints", "CA:FALSE", true))
      certificate.add_extension(ef.create_extension("keyUsage", "digitalSignature,keyEncipherment", true))
      certificate.add_extension(ef.create_extension("extendedKeyUsage", "serverAuth", true))  # Set critical
      certificate.add_extension(ef.create_extension("subjectKeyIdentifier", "hash", false))
      certificate.add_extension(ef.create_extension("authorityKeyIdentifier", "keyid,issuer", false))
      certificate.add_extension(ef.create_extension("subjectAltName", "DNS:#{domain},DNS:www.#{domain}", false))  # Add multiple SANs
